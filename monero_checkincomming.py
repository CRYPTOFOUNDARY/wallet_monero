...
def addtounconfirmed(amount, userid, txid):
    ...
    :return:
    """
    unconfirmedtable = db.session.query(MoneroUnconfirmed).filter_by(userid=userid).first()
...
def removeunconfirmed(userid, txid):
    ...
    try:
        unconfirmeddelete = db.session.query(MoneroUnconfirmed).filter_by(userid=userid).first()
        if unconfirmeddelete.txid1 == txid:
            unconfirmeddelete.txid1 = ''
            unconfirmeddelete.unconfirmed1 = 0
            db.session.add(MoneroUnconfirmed)
            db.session.commit()
...
def getbalanceUnconfirmed(userid):
    ...
    wallet = MoneroWallet.query.filter_by(userid=userid).first()
        ...
        if "result" in response_json:
            if "payments" in response_json["result"]:
                for incpayments in response_json["result"]["payments"]:
                    ...
                    getuserswallet = db.session.query(MoneroWallet).filter_by(paymentid=userpaymentid).first()
                    ...
                    if getuserswallet is not None:
                        ...
                        y = db.session.query(MoneroUnconfirmed).filter_by(txid=hashid).first()
                        if y:
                            pass
                        else:
...
def get_money(amount):
    """
    This function converts monero amount to a more legible btc amount
    :param amount:
    :return:
    """
    try:
