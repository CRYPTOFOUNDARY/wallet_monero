...
from app.models import\
    MoneroWalletWork, MoneroBlockheight, MoneroWallet
...
def sendCoin(f, sendto, amount, paymentid):
    ...
    try:
        ...
        getuserswallet = db.session.query(MoneroWallet).filter_by(userid=f.userid).first()
...
def get_amount(amount):
    ...
    try:
        ...
        print("monero-send-getamount error")
        print(str(e))
...
def main():
    ...
    work = work.filter(MoneroWalletWork.type != 0)
    work = work.filter(MoneroWalletWork.shard == shard)
    work = work.order_by(MoneroWalletWork.created.desc())
    walletwork = work.all()
